...
VARIABLE votedFor
\* The index of the last snapshot that was stored
VARIABLE snapshotIndex
serverVars == <<currentTerm, state, votedFor, snapshotIndex>>
...
InitGlobalVars == currentActive = Server               
Init == /\ InitServerVars
        /\ InitCandidateVars
        /\ InitLeaderVars
        /\ InitLogVars
...
Restart(i) ==
    ...
    /\ UNCHANGED <<currentTerm, votedFor, log, currentActive, snapshotIndex, currentActive>>
...
ClientRequest(i, v) ==
    ...
    /\ UNCHANGED <<serverVars, candidateVars,
                   leaderVars, commitIndex, currentActive>>
...
AdvanceCommitIndex(i) ==
    ...
    /\ UNCHANGED <<serverVars, candidateVars, leaderVars, log, currentActive>>

\* Update the snapshot index of the process i
...
IsCurrentlyActive(i) == /\ i \in currentActive
...
HandleRequestVoteResponse(i, j, term, grant) ==
    LET cTerm == (IF term > currentTerm[i] THEN term ELSE currentTerm[i])
        ...
        vFor == (IF term > currentTerm[i] THEN  Nil ELSE votedFor[i])
    IN  /\ IsCurrentlyActive(i)
        /\ term < cTerm => UNCHANGED <<serverVars, candidateVars, leaderVars, logVars, currentActive>>
        ...
        IN  /\ term <= cTerm
            /\  \/  /\ \* reject request
                        ...
                        \/  /\ term = cTerm
                            ...
                            /\ \lnot logOk
                    /\ IF term > currentTerm[i] THEN UpdateTerm(i, term) ELSE UNCHANGED <<serverVars, currentActive>>
                    /\ UNCHANGED <<logVars, snapshotIndex, currentActive>>
                \/ \* return to follower state
                    /\ term = cTerm
                    /\ cState = Candidate
                    /\ state' = [state EXCEPT ![i] = Follower]
                    /\ IF term > currentTerm[i] THEN (currentTerm' = [currentTerm EXCEPT  ![i] = cTerm] /\ votedFor' = [votedFor EXCEPT ![i] = vFor]) ELSE UNCHANGED <<currentTerm, votedFor, snapshotIndex, currentActive>>
                    /\ UNCHANGED <<logVars, snapshotIndex, currentActive>>
                \/ \* accept request
                    /\ term = cTerm
                    /\ cState = Follower
                    ...
                    /\  LET index == pLogIndex + 1
                        IN  \* already done with request or
                            ...
                            /\ commitIndex' = [commitIndex EXCEPT ![i] =
                                                    cIndex]
                            /\ IF term > currentTerm[i] THEN UpdateTerm(i, term) ELSE UNCHANGED <<serverVars, currentActive>>
                            /\ UNCHANGED <<log, snapshotIndex, currentActive>>
            /\ UNCHANGED <<candidateVars, leaderVars, snapshotIndex, currentActive>>)
                    ...
                    /\ LET index == pLogIndex + 1
                        IN  \/ \* already done with request
                            ...
                            \/ \* conflict: remove 1 entry
                                ...
                                /\ IF term > currentTerm[i] THEN UpdateTerm(i, term) ELSE UNCHANGED <<serverVars, currentActive>>
                                /\ UNCHANGED <<commitIndex, snapshotIndex, currentActive>>
                            \/ \* no conflict: append entry
                                ...
                                /\ IF term > currentTerm[i] THEN UpdateTerm(i, term) ELSE UNCHANGED <<serverVars, currentActive>>
                                /\ UNCHANGED <<commitIndex, snapshotIndex, currentActive>>
            /\ UNCHANGED <<candidateVars, leaderVars, currentActive, snapshotIndex>>)
...
HandleAppendEntriesResponse(i, j, term, success, mIndex) ==
    ...
    IN  /\ IsCurrentlyActive(i)
        /\  term < cTerm => UNCHANGED <<candidateVars, leaderVars, logVars, snapshotIndex, currentActive>>
        /\  /\ term = cTerm
            /\  \/  /\ success \* successful
                ...
                /\  LET \* The set of servers that agree up through index.
                    ...
                    newCommitIndex ==
                        ...
                        ELSE
                            commitIndex[i]
                    IN commitIndex' = [commitIndex EXCEPT ![i] = newCommitIndex]
            /\ UNCHANGED <<candidateVars, log, currentActive, snapshotIndex>>
        /\  IF term > currentTerm[i] THEN UpdateTerm(i, term) ELSE UNCHANGED <<serverVars, currentActive>>
...
Next == \/ \E i \in Server : Restart(i)
        ...
        \/ \E i \in Server, si \in LogIndices : UpdateSnapshotIndex(i, si)
